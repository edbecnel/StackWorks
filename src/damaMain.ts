...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  // Seed default piece + board appearance when missing (first run / storage cleared).
  if (isCheckers) {
    try {
      if (!localStorage.getItem(LS_THEME_KEY)) localStorage.setItem(LS_THEME_KEY, "classic");
      if (!localStorage.getItem(LS_OPT_KEYS.checkerboardTheme)) {
        localStorage.setItem(LS_OPT_KEYS.checkerboardTheme, "classic");
      }
    } catch {
      // ignore
    }
