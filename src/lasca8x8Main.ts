...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  const useCheckered8x8 = readOptionalBoolPref(LS_OPT_KEYS.board8x8Checkered) ?? true;
  const svgAsset =
  ...
  if (import.meta.env && import.meta.env.DEV) {
    ...
    w.__random = (totalPerSide: number = 11, toMove: Player = "B", testMode?: string) => {
      ...
      const rng = createPrng(`debug.__random:lasca8x8:${totalPerSide}:${toMove}:${testMode ?? ""}`);
